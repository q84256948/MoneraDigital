...
| balance | DECIMAL(32,16) | NOT NULL | 账户余额（已扣款，不含冻结） |
| frozen_balance | DECIMAL(32,16) | NOT NULL | 冻结余额（提现等待中的冻结金额） |
...
| amount | DECIMAL(32,16) | NOT NULL | 变动金额（正数增加，负数减少） |
...
| 字段名 | 类型 | 约束 | 说明 |
|--------|------|------|------|
| id | BIGINT | PK, AI | 对账记录ID |
| check_time | DATETIME | Index | 对账时间 |
| type | VARCHAR(32) | Index | 对账类型: BALANCE_MATCH, JOURNAL_VERIFY等 |
| user_total | DECIMAL(32,16) | | 用户账户总额 |
| system_balance | DECIMAL(32,16) | | 系统账户余额 |
| difference | DECIMAL(32,16) | | 差异金额 |
...
| verified_at | DATETIME | Index | 认证通过时间 |
| expires_at | DATETIME | Index | 认证过期时间 |
| created_at | DATETIME | DEFAULT NOW | 创建时间 |
| updated_at | DATETIME | DEFAULT NOW | 更新时间 |
...
| result_data | JSON | | 成功时返回的结果数据（如order_id） |
| error_message | VARCHAR(255) | | 失败时的错误信息 |
| created_at | DATETIME | DEFAULT NOW | 请求创建时间 |
| completed_at | DATETIME | | 请求完成时间 |
| ttl_expire_at | DATETIME | Index | 记录过期时间（通常为24小时后） |
...
| amount | DECIMAL(32,16) | NOT NULL | 划转金额 |
...
| apy | DECIMAL(10,4) | NOT NULL | 年化收益率（如0.0700表示7%） |
| duration | INT | NOT NULL | 期限（天数） |
| min_amount | DECIMAL(32,16) | NOT NULL | 起购金额 |
| max_amount | DECIMAL(32,16) | NOT NULL | 单人限额 |
| total_quota | DECIMAL(32,16) | NOT NULL | 总额度 |
| sold_quota | DECIMAL(32,16) | NOT NULL | 已售额度（乐观锁更新） |
| status | TINYINT | Index | 状态: 1-待上架, 2-募集中, 3-已售罄, 4-已结束 |
...
| amount | DECIMAL(32,16) | NOT NULL | 申购本金 |
| principal_redeemed | DECIMAL(32,16) | | 已赎回本金 |
| interest_expected | DECIMAL(32,16) | NOT NULL | 预期总利息（快照字段，申购时计算好） |
| interest_paid | DECIMAL(32,16) | NOT NULL | 已派发利息 |
| start_date | DATE | NOT NULL | 起息日 |
| end_date | DATE | Index | 到期赎回日（用于定时任务扫描） |
| auto_renew | TINYINT | | 是否开启自动续期: 0-否, 1-是 |
| status | TINYINT | Index | 状态: 0-处理中, 1-持有中(计息), 2-已赎回/结束, 3-失败, 4-已续期 |
| renewed_from_order_id | BIGINT | Index | 续期来源的订单ID（如果是续期产品） |
| renewed_to_order_id | BIGINT | Index | 续期到的新订单ID |
| created_at | DATETIME | DEFAULT NOW | 申购时间 |
| updated_at | DATETIME | DEFAULT NOW | 更新时间 |
...
| user_id | BIGINT | Index | 用户ID（冗余字段，方便查询） |
| amount | DECIMAL(32,16) | NOT NULL | 当次产生/发放的利息 |
| type | TINYINT | Index | 类型: 1-每日计提(仅记录), 2-实际发放(入账) |
...
| verified | TINYINT | Index | 是否已验证: 0-否, 1-是 |
...
| verified | TINYINT | Index | 是否通过验证: 0-否, 1-是 |
...
- [x] 所有表都有主键
- [x] 所有时间字段都有 DEFAULT CURRENT_TIMESTAMP
- [x] 所有金额字段都使用 DECIMAL(32,16)
- [x] frozen_balance 在 account 表中定义
- [x] 所有高频查询字段都有索引
- [x] 所有 unique 字段都有 unique key
- [x] 所有表都有中英文注释
- [x] 脚本可以直接执行（包括所有 IF NOT EXISTS）
